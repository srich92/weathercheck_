...
let formSubmitHandler = function(event) {
    ...
    if (cityName) {
        ...
        errorContainerEl.innerHTML = "Please enter the name of the city !"
        return;
    }
}
...
let getWeather = function(lat, lon) {
    let apiUrl = "http://api.openweathermap.org/data/2.5/onecall?lat=" + lat + "&lon=" + lon + "&units=imperial&appid=" + apiKey
    fetch(apiUrl)
        .then(function(res) {
            if (res.ok) {
                return res.json();
...
let displayWeather = function(data) {
    let apiUrl = "http://api.openweathermap.org/data/2.5/onecall?lat=" + data.lat + "&lon=" + data.lon + "&limit=1&appid=" +apiKey
    let iconLink = "http://openweathermap.org/img/w/" + data.current.weather[0].icon + ".png"
    fetch(apiUrl)
        .then(function(res) {
            return res.json();
        })
...
let displayForecast = function(data) {
    ...
    for (j = 0; j < 5; j++) {
        ...
        let iconLink = "http://openweathermap.org/img/w/" + currentData.weather[0].icon + ".png"
